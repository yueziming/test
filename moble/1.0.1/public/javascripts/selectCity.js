var provinceList = [{
    ...
    },
    {
        ...
        cityList: [
            ...
            { name: '直辖县', areaList: ['崇明县', "城桥镇"] }
        ]
    },
    {
        name: '天津',
        cityList: [
            { name: '市辖区', areaList: ["和平区", "河东区", "河西区", "南开区", "河北区", "红桥区", "塘沽区", "汉沽区", "大港区", "东丽区", "西青区", "津南区", "北辰区", "武清区", "宝坻区", "静海区"] },
            { name: '直辖县', areaList: ['宁河县', '静海县', '蓟县', "芦台镇", "静海县", "静海镇"] }
        ]
    },
    {
        name: '重庆',
        cityList: [
            { name: '市辖区', areaList: ["渝中区", "大渡口区", "江北区", "沙坪坝区", "九龙坡区", "南岸区", "北碚区", "万盛区", "双桥区", "渝北区", "巴南区", "万州区", "涪陵区", "黔江区", "长寿区"] },
            { name: '直辖县', areaList: ["綦江县", "潼南县", "铜梁县", "大足县", "荣昌县", "璧山县", "垫江县", "武隆县", "丰都县", "城口县", "梁平县", "开县", "巫溪县", "巫山县", "奉节县", "云阳县", "忠县", "石柱土家族自治县", "彭水苗族土家族自治县", "酉阳土家族苗族自治县", "秀山土家族苗族自治县"] },
            { name: '直辖市', areaList: ['江津市', '合川市', '永川市', '南川市'] }
        ]
    },
    {
        name: '四川',
    ...
    },
    {
        name: '台湾',
        cityList: [
            { name: '市、县级市', areaList: ["台北", "高雄", "台中", "花莲", "基隆", "嘉义", "金门", "连江", "苗栗", "南投", "澎湖", "屏东", "台东", "台南", "桃园", "新竹", "宜兰", "云林", "彰化"] }
        ]
    },
    {
        name: '香港',
 ...
 */
function _init_Data() {
    var sel = ['province', 'city', 'county'];
    var html = '<option value="{value}">{name}</option>',
        tempHtml = html.replace('{value}', '').replace('{name}', '请选择省份');
    for (var i = 0; i < provinceList.length; i++) {
        tempHtml += html.replace('{value}', provinceList[i].name).replace('{name}', provinceList[i].name);
    }
    $('#' + sel[0]).html(tempHtml);
    var nsel = ['请选择城市', '请选择县、县级市'];
    for (var chans = 0; chans < sel.length; chans++) {
        if (chans != sel.length - 1) {
            (function(chans) {
                $('#' + sel[chans + 1]).html(html.replace('{value}', '').replace('{name}', nsel[chans]));
                $('#' + sel[chans]).change(function() {
                    seleChange($(this).val(), chans, true);
                });
            })(chans)
        }
    }
 ...
 * 选择城市
 */
function seleChange(values, num, flag) {
    var sel = ['province', 'city', 'county'];
    var html = '<option value="{value}">{name}</option>',
        tempHtml = html.replace('{value}', '').replace('{name}', '请选择市、县级市');
    if (num == 0) {
        initPageData.city = values;
        for (var i = 0; i < provinceList.length; i++) {
            if (provinceList[i].name == values) {
                var temp = provinceList[i];
                for (var s = 0; s < temp.cityList.length; s++) {
                    var thisTemp = temp.cityList[s];
                    tempHtml += html.replace('{value}', thisTemp.name).replace('{name}', thisTemp.name);
                }
            }
        }
    } else {
        tempHtml = html.replace('{value}', '').replace('{name}', '请选择地区');
        for (var i = 0; i < provinceList.length; i++) {
            if (provinceList[i].name == initPageData.city) {
                var thisTemp = provinceList[i];
                for (var sm = 0; sm < thisTemp.cityList.length; sm++) {
                    var tempArry = thisTemp.cityList[sm];
                    if (tempArry.name == values) {
                        for (var s = 0; s < tempArry.areaList.length; s++) {
                            tempHtml += html.replace('{value}', tempArry.areaList[s]).replace('{name}', tempArry.areaList[s]);
                        }
                    }
                }
            }
    ...
    $('#' + sel[num + 1]).html(tempHtml);
};

function _init_Update(a, b, c) {
    $('#province').val(a).change();
    $('#city').val(b).change();
    $('#county').val(c).change();
}