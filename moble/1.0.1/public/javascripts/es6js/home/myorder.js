...
$(function () {
    ...
    function ajaxForm(flag) {
        var baseAjax = {
            url: __WEBPACK_IMPORTED_MODULE_1__api__["a" /* API */].ORDERLIST
        };
        var condition = {
        ...
        __WEBPACK_IMPORTED_MODULE_3__commonAjax_js___default.a.ajax(baseAjax, condition, function (res) {
            if (res.Data.List) {
                ...
                for (var item in data) {
                    htmlstr += '<section class="order-list" data-id="' + data[item].OrderID + '" data-price="' + (data[item].OrderPrice || 0) + '">' + '<div class="order-list-header">' + '<span>订单编号&nbsp;&nbsp;</span><span class="order-no">' + data[item].OrderNo + '</span>';
                    ...
                    for (var index in data[item].DetailList) {
                        htmlstr += '<div class="order-list-content" data-id="' + data[item].DetailList[index].OrderDetailID + '">' + '<div class="list-content">' + '<div class="cols5 float-left">' + '<span class="order-list-title">' + data[item].DetailList[index].ServiceName + '</span></div>' + '<div class="cols2 float-left">' + '<span class="order-list-quantity">X&nbsp;' + data[item].DetailList[index].Count + '</span></div>' + '<div class="cols3 float-left">' + '<p class="text-right" style="margin-bottom:.02rem;">' + '<span class="color-red fweight600">￥' + data[item].DetailList[index].DiscountPrice + '</span></p>' + '<p class="text-right">' + '<span class="fweight600 inlinethrough">￥' + data[item].DetailList[index].DisplayPrice + '</span></p></div></div>' + '<div class="clearboth"></div>';
                    }
                    htmlstr += '<div class="order-list-operate text-right">' + '共' + data[item].Count + '项服务&nbsp;<span class="order-pay-title">合计:</span><span class="color-red fweight600">￥' + (data[item].OrderPrice || 0.00) + '</span></div>' + '<div class="order-list-operate text-right">';
...
function bindClick() {
    ...
    $('main .cancel-btn').off('click').on('click', function () {
        ...
        __WEBPACK_IMPORTED_MODULE_0__msgBox_js___default.a.comFirmMsg(0, "确定取消订单吗？", "确定", "取消", function () {
            var baseAjax = {
                url: __WEBPACK_IMPORTED_MODULE_1__api__["a" /* API */].CANCELORDER
            };
            var condition = {
                OrderID: $this.attr("data-id")
            };
            __WEBPACK_IMPORTED_MODULE_3__commonAjax_js___default.a.ajax(baseAjax, condition, function (res) {
                if (res) {
                    __WEBPACK_IMPORTED_MODULE_0__msgBox_js___default.a.tipMsgBox('订单已取消', 1000);
                    $this.remove();
                }
            });
        }, function () {
    ...
    $('main .btn-pay').off("click").on('click', function () {
        localStorage.setItem("orderNo", $(this).parents("section").attr("data-id"));
        //存储数据给结算
        var $con = $(this).parents("section").find(".order-list-title");
        var bodyTitle = [];
        var Totlafees = $(this).parents("section").attr("data-price") || 0;
